...
# anwen hu 2020/5/14
# used for sc learning
@registry.register_metric("textcaps_cider")
class TextCapsCider(BaseMetric):
    def __init__(self):
        super().__init__('textcaps_cider')
        self.gt_key = 'ref_strs'
        import pythia.utils.m4c_evaluators as evaluators
        self.evaluator = evaluators.TextCapsCiderEvaluator()
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        batch_size = sample_list.context_tokens_enc.size(0)
        pred_answers = model_output["scores"].argmax(dim=-1)
        pred_answers_sample = model_output["scores_sample"].argmax(dim=-1)
        context_tokens_enc = sample_list.context_tokens_enc.cpu().numpy()
        gt_answers_enc = sample_list[self.gt_key].cpu().numpy()
        answer_space_size = answer_processor.get_true_vocab_size()
        ...
        for idx in range(batch_size):
            ...
            pred_answer = ' '.join(answer_words).replace(" 's", "'s")
            for answer_sample_id in pred_answers_sample[idx].tolist():
                if answer_sample_id >= answer_space_size:
                    answer_sample_id -= answer_space_size
                    answer_words_sample.append(
                        word_tokenize(context_tokens[answer_sample_id])
                    )
                else:
                    if answer_sample_id == answer_processor.EOS_IDX:
                        break
                    answer_words_sample.append(
                        answer_processor.answer_vocab.idx2word(answer_sample_id)
                    )

            pred_answer_sample = ' '.join(answer_words_sample).replace(" 's", "'s")
            gt_answers = dec_bytes2obj(gt_answers_enc[idx])
            predictions.append({
                "pred_answer": pred_answer,
                "gt_answers": gt_answers,
            ...
            predictions_sample.append({
                "pred_answer": pred_answer_sample,
                "gt_answers": gt_answers,
            })
        cider_score = self.evaluator.eval_pred_list(predictions)
        cider_score_sample = self.evaluator.eval_pred_list(predictions_sample)
        print('pythia/modules/metrics.py cider_score', cider_score)
        print('pythia/modules/metrics.py cider_score average', np.mean(cider_score))
        print('pythia/modules/metrics.py cider_score_sample', cider_score_sample)
        print('pythia/modules/metrics.py cider_score_sample average', np.mean(cider_score_sample))
        reward = cider_score_sample-cider_score
        # print(pred_answers_sample.size())
        reward = np.repeat(reward[:, np.newaxis], pred_answers_sample.data.cpu().numpy().shape[1], 1)
        # print('reward:', torch.tensor(reward).cuda().size())
        model_output["reward"] = torch.tensor(reward).cuda()


# anwenhu 2021/2/24
@registry.register_metric("controlvqa_accuracy")
class ControlVQAccuracy(BaseMetric):
    def __init__(self):
        super().__init__("controlvqa_accuracy")
        self.name = "controlvqa_accuracy"
        import pythia.utils.m4c_evaluators as evaluators
        self.evaluator = evaluators.TextCapsBleu4Evaluator()

    def calculate(self, sample_list, model_output, *args, **kwargs):
        batch_size = model_output["vqa_scores"].size(0)
        pred_answers = model_output["vqa_scores"].cpu().numpy()  # batch * max_question_num * max_ocr_length
        # print('pythia/modules/metrics.py pred_answers: ', pred_answers.shape)
        gt_answers = sample_list["ans_target"].cpu().numpy() # batch * max_question_num * max_ocr_length
        # print('pythia/modules/metrics.py gt_answers: ', gt_answers.shape)
        max_question_num = gt_answers.shape[1]
        all_num = 0
        true_num = 0
        for idx in range(batch_size):
            ...
            for i in range(max_question_num):
                if np.sum(gt_answer[i]) > 0:  # sum==0 means gt is a 0 vector
                    pred_index = pred_answer[i].argmax(axis=-1)
                    gt_index = gt_answer[i].argmax(axis=-1)
                    all_num += 1
                    if pred_index == gt_index:
                        true_num += 1
        accuracy = round(float(true_num)/all_num, 4)
        accuracy = torch.tensor(accuracy).cuda()
        return accuracy
