...
class Losses(nn.Module):
    ...
    def __init__(self, loss_list):
        ...
        for loss in loss_list:
            print('/pythia/modules/losses.py loss: ', loss)
    ...
    def forward(self, sample_list, model_output, *args, **kwargs):
        ...
        # print('pythia/modules/losses.py output', output)  # dict {loss_name:loss_value}
...
class M4CDecodingBCEWithMaskLoss(nn.Module):
    def __init__(self):
        ...
        print('pythia/modules/losses.py use M4CDecodingBCEWithMaskLoss loss')
    ...
    def forward(self, sample_list, model_output):
        ...
        # anwenhu 2021/3/10 debug
        """logit_scores = torch.sigmoid(scores)
        target_logit_scores = torch.sum(torch.mul(targets, logit_scores), dim=-1)
        print('target logit scores ', target_logit_scores)
        print('target index ', torch.nonzero(targets))"""
        losses = F.binary_cross_entropy_with_logits(
            scores, targets, reduction="none"
        )
        # print('losses(after -log) ', torch.sum(losses, dim=-1))
        # print('target index ', torch.nonzero(targets))
        losses *= loss_mask.unsqueeze(-1)
        count = torch.max(torch.sum(loss_mask), self.one.to(losses.device))
        # print('loss sum ', torch.sum(losses))
        # print('count ', count)
        loss = torch.sum(losses) / count
        # print('pythia/modules/losses.py M4CDecodingBCEWithMaskLoss: ', loss)
        return loss

# anwen hu 2020/11/15
@registry.register_loss("m4c_decoding_ce_with_mask")
class M4CDecodingCEWithMaskLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.])
        print('pythia/modules/losses.py use M4CDecodingCEWithMaskLoss loss')

    def forward(self, sample_list, model_output):
        scores = model_output["scores"]
        targets = sample_list["targets"]
        loss_mask = sample_list["train_loss_mask"]  # batch * 30
        assert scores.dim() == 3 and loss_mask.dim() == 2
        assert scores.shape == targets.shape
        # print('pythia/modules/losses.py targets.shape', targets.shape) batch * 30 * vocab_size
        # print('pythia/modules/losses.py targets[0][0].sum', torch.sum(targets[0][0]))

        scores = F.log_softmax(scores, dim=2)  # batch * 30 * vocab_size (vizwiz: 7283+50)
        # print('scores(after log and softmax) ', scores)
        losses = torch.mul(targets, scores)*-1  # batch * 30 * vocab_size
        # print('scores(after mul targets) ', torch.sum(losses, dim=-1))
        # print('target index ', torch.nonzero(targets))
        losses *= loss_mask.unsqueeze(-1)
        count = torch.max(torch.sum(loss_mask), self.one.to(losses.device))
        # print('loss sum ', torch.sum(losses))
        # print('count ', count)
        loss = torch.sum(losses) / count
        # anwenhu 2021/3/11 when there is a <unk> in target, the loss for unk will be very huge 1e+10
        # print('pythia/modules/losses.py M4CDecodingBCEWithMaskLoss: ', loss)
        return loss
