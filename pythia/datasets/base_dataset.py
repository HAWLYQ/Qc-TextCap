...
class BaseDataset(Dataset):
    ...
    def init_processors(self):
        ...
        for processor_key, processor_params in self.config.processors.items():
            ...
            # print('pythia/datasets/base_dataset.py reg_key', reg_key) m4c_textcaps_context_processor ...
            reg_check = registry.get(reg_key, no_warning=True)
            # print('pythia/datasets/base_dataset.py reg_check ', reg_check) pythia.datasets.processors.Processor

            if reg_check is None:
                # print('reg_check is None'), enter here when dataset is train set
                processor_object = Processor(processor_params, **extra_params)  # initiate processor
                # print('pythia/datasets/base_dataset.py init processor')
                setattr(self, processor_key, processor_object)
                registry.register(reg_key, processor_object)
            else:
                # print('reg_check is not None'), enter here when dataset is dev/text set
                # set self.#processor_name# = Processor, not initiated here, e.g. self.answer_processor = Processor
