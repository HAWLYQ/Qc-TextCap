...
class Processor:
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        # print('pythia/datasets/processor.py processor_class', processor_class)
...
class BertTokenizerProcessor(BaseProcessor):
    ...
    """
    def __init__(self, config, *args, **kwargs):
        # print('pythia/datasets/processors.py bert_tokenizer init, max length', config.max_length)
        self.max_length = config.max_length
    ...
    def __call__(self, item):
        ...
        # print('pythia/datasets/processors.py bert_tokenizer item[question]', item['question'])
        indices = self.bert_tokenizer.encode(
            item['question'], add_special_tokens=True)  # cls_id + token_ids + sep_id
        # print('pythia/datasets/processors.py bert_tokenizer indices', indices)
        indices = indices[:self.max_length]
        token_inds[:len(indices)] = torch.tensor(indices)
        token_num = torch.tensor(len(indices), dtype=torch.long)
        # print('pythia/datasets/processors.py bert_tokenizer token_inds', token_inds)
        # print('pythia/datasets/processors.py bert_tokenizer token_num', token_num)
...
@registry.register_processor("bert_tokenizer_control")
class BertTokenizerControlProcessor(BaseProcessor):
    """
    Tokenize a text string with BERT tokenizer
    """
    def __init__(self, config, *args, **kwargs):
        # print('pythia/datasets/processors.py bert_tokenizer init, max length', config.max_length)
        self.max_length = config.max_length
        self.max_que_num = config.max_question_num
        self.bert_tokenizer = BertTokenizer.from_pretrained(
            'bert-base-uncased')
        assert self.bert_tokenizer.encode(self.bert_tokenizer.pad_token) == [0]
        # print('pythia/datasets/processor.py bert tokenizer [unused0] id', self.bert_tokenizer.encode('[unused0]'))
        # print('pythia/datasets/processor.py bert tokenizer [unused0] id', self.bert_tokenizer.decode([1]))
        print('pythia/datasets/processor.py bert tokenizer [CLS] id', self.bert_tokenizer.encode('[CLS]'))  # cls=101
        print('pythia/datasets/processor.py bert tokenizer [SEP] id', self.bert_tokenizer.encode('[SEP]'))  # cls=102
        self.get_qgen_inds = getattr(config, 'get_qgen_inds', False)
        if self.get_qgen_inds:
            print('computing question generation indices in bert tokenizer')
    ...
    def __call__(self, item, fetch_ans_index=False):
        # [PAD] in self.bert_tokenizer is zero (as checked in assert above)
        token_inds = torch.zeros(self.max_length, dtype=torch.long)
        # print('pythia/datasets/processors.py bert_tokenizer item[question]', item['question'])
        indices = self.bert_tokenizer.encode(
            item['question'], add_special_tokens=False)
        indices = [101]+indices  # add <CLS> at the index 0
        indices = indices[:self.max_length]  # max length=20
        # print('pythia/datasets/processors.py bert_tokenizer indices', indices)
        if fetch_ans_index:
            # anwen hu 2021/2/24 get [SEP] index
            ans_index = torch.zeros(self.max_que_num, dtype=torch.long)  # pad with 0, 0 refer to the CLS token
            que_idx = 0
            for i, indice in enumerate(indices):
                if indice == 102:  # SEP
                    ans_index[que_idx] = i
                    que_idx += 1
            # print('pythia/datasets/processors.py bert_tokenizer ans_index', ans_index)
        token_inds[:len(indices)] = torch.tensor(indices)
        token_num = torch.tensor(len(indices), dtype=torch.long)
        # print('pythia/datasets/processors.py bert_tokenizer token_inds', token_inds)
        # print('pythia/datasets/processors.py bert_tokenizer token_num', token_num)
        if fetch_ans_index:
            results = {'token_inds': token_inds, 'token_num': token_num, 'ans_index': ans_index}
        else:
            results = {'token_inds': token_inds, 'token_num': token_num}
...
class M4CAnswerProcessor(BaseProcessor):
    ...
    def match_answer_to_vocab_ocr_seq(
        ...
        answer_word_matches = []
        # print('pythia/datasets/processors.py answer_words', answer_words)
        for word in answer_words:
            ...
            if len(matched_inds) == 0:
                if self.match_answer_to_unk:  # True for captioning
                    matched_inds.append(vocab2idx_dict.get('<unk>'))
                else:
                    return []

            answer_word_matches.append(matched_inds)
        # print('pythia/datasets/processors.py answer_word_matches', answer_word_matches)
        ...
        # print('pythia/datasets/processors.py idx_seq_list', idx_seq_list)
        return idx_seq_list
    ...
    def __call__(self, item):
        answers = item["answers"]
        answers = [
            # simple_word processor: lower, remove , ? , add space before 's
        ...
        scores = torch.zeros(
            self.max_copy_steps,  # max decoding step
            self.get_vocab_size(),
            dtype=torch.float
        )
        ...
        for idx, token in enumerate(item["context_tokens"]):  # context_tokens: ocr tokens with <PAD>
            ocr2inds_dict[token].append(idx)  # there may be same ocr tokens
        answer_dec_inds = [
        ...
        for answer, idx_seq_list in zip(answers, answer_dec_inds):
            ...
            score = unique_answer2score[answer] # 1.0 for captioning
        ...
        if len(all_idx_seq_list) > 0:
            ...
            # print("pythia/datasets/processsors.py idx_seq", idx_seq) no BOS or EOS
            ...
            # print("pythia/datasets/processsors.py train_prev_inds", train_prev_inds) # contain BOS but not EOS
            # anwen hu 2020/11/16 reset scores[0] to a one-hot vector;
            # anwen hu 2020/11/16 move this part to HUM4CCaptionProcessor
            first_word_score = torch.zeros_like(scores[0])
            first_word_score[idx_seq[0]] = 1.
            scores[0] = first_word_score

        else:
            idx_seq = ()
        answer_info = {
            'answers': answers,
            'answers_scores': scores,  # contain EOS but not EOS
...
@registry.register_processor("m4c_azure_caption")
class M4CAzureCaptionProcessor(M4CCaptionProcessor):
    def __init__(self, config, *args, **kwargs):
        super().__init__(config, *args, **kwargs)
        self.ignore_unk_target = config.ignore_unk_target
    ...
    def __call__(self, item):
        ...
        # match answers to fixed vocabularies and OCR tokens.
        ocr2inds_dict = defaultdict(list)
        # for azure ocr tokens, there may be multiple word in one token
        for idx, token in enumerate(item["context_tokens"]):  # context_tokens: ocr tokens with <PAD>
            sub_tokens = token.split(' ')
            for sub_token in sub_tokens:
                ocr2inds_dict[sub_token].append(idx)  # there may be same ocr tokens
        # print('pythia/datasets/processors.py id 3: ', self.answer_vocab.itos[3])  # <unk>
        answer_dec_inds = [
            self.match_answer_to_vocab_ocr_seq(
                a, self.answer_vocab.word2idx_dict, ocr2inds_dict
            ) for a in answers
        ...
        if len(all_idx_seq_list) > 0:
            # sample a random decoding answer sequence for teacher-forcing
            idx_seq = all_idx_seq_list[np.random.choice(len(all_idx_seq_list))]
            # for azure ocr, there may be multiple word from one ocr token,
            # so there may be successive same id for ocr vocab, thus only keep one
            idx_seq = [x[0] for x in groupby(idx_seq)]
            dec_step_num = min(1+len(idx_seq), self.max_copy_steps)  # +1 means a BOS token
            train_loss_mask[:dec_step_num] = 1.
            ...
            for t in range(1, dec_step_num):
                ...
                scores[t, score_idx] = 1.
                # anwen hu 2021/3/11
                if score_idx == 3 and self.ignore_unk_target: # <unk>
                    train_loss_mask[t] = 0.0
            # print("pythia/datasets/processsors.py train_prev_inds", train_prev_inds) # contain BOS but not EOS
            # anwen hu 2020/11/16 reset scores[0] to a one-hot vector;
            # anwen hu 2020/11/16 move this part to HUM4CCaptionProcessor
            first_word_score = torch.zeros_like(scores[0])
            first_word_score[idx_seq[0]] = 1.
            # anwen hu 2021/3/11
            if idx_seq[0] == 3 and self.ignore_unk_target:  # <unk>
                train_loss_mask[0] = 0.0
            scores[0] = first_word_score
        ...
        answer_info = {
            'answers': answers,
            'answers_scores': scores,  # contain EOS but not EOS, dec_step_num * vocab_size
            'sampled_idx_seq': idx_seq,
            'train_prev_inds': train_prev_inds,
            'train_loss_mask': train_loss_mask,
        }
...
# anwen hu, revise the caption processor
@registry.register_processor("hu_m4c_caption")
class HUM4CCaptionProcessor(M4CAnswerProcessor):
    def __init__(self, config, *args, **kwargs):
        super().__init__(config, *args, **kwargs)
        import re
        self.SENTENCE_SPLIT_REGEX = re.compile(r"(\W+)")
    ...
    def match_answer_to_vocab_ocr_seq(
        ...
        # answer_words = self.tokenize(answer)
        # answer has been tokenized by spacy during preprocessing
    ...
    def __call__(self, item):
        # print('pythia/datasets/processors.py HUM4CCaptionProcessor answer_processor')
        answers = item["answers"]
        answers_raw = item['answers']
        # use pythia word_tokenize
        answers_raw = [
            self.answer_preprocessor({"text": a})["text"] for a in answers_raw
        ]
        print('pythia/datasets/processors.py tokenized answer', answers[0].split(' '))
        print('pythia/datasets/processors/py tokenized answer_raw',  self.tokenize(answers_raw[0]))

        assert len(answers) == 1
        assert len(answers) == self.num_answers
        # print('pythia/datasets/processors.py answers', answers)
        # Step 2: fill the first step soft scores for tokens
        scores = torch.zeros(
            self.max_copy_steps,  # max decoding step
            self.get_vocab_size(),
        ...
        all_idx_seq_list = self.match_answer_to_vocab_ocr_seq(
                answers[0], self.answer_vocab.word2idx_dict, ocr2inds_dict
            )
        # train_prev_inds is the previous prediction indices in auto-regressive
        # decoding
        train_prev_inds = torch.zeros(self.max_copy_steps, dtype=torch.long)
        ...
        if len(all_idx_seq_list) > 0:
            ...
            idx_seq = ()
        # anwen hu 2020/11/10 debug, assert every vector in scores is an one-hot vector
        # print('pythia/datasets/processors.py scores[0].sum', torch.sum(scores[0]))
        """for t in range(self.max_copy_steps):
            try:
                assert torch.sum(scores[t]) == 1.0 or torch.sum(scores[t]) == 0.0
            except AssertionError:
                print(torch.sum(scores[t]))"""
        answer_info = {
            'answers': answers,
            'answers_scores': scores,  # contain EOS but not EOS
            'sampled_idx_seq': idx_seq,
