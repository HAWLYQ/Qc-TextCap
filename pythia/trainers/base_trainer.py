...
from pythia.modules.metrics import TextCapsCider

#anwen hu 2020/5/15 used for sc leaning
...
class RewardCriterion(nn.Module):
    ...
    def forward(self, input, seq, reward):
        # input: batch * 30 logprob for sampled id
        # seq: batch * 30  id
        # reward: batch * 30
        input = to_contiguous(input).view(-1).double()
        reward = to_contiguous(reward).view(-1).double()
        mask = (seq>0).double()
        mask = to_contiguous(torch.cat([mask.new(mask.size(0), 1).fill_(1), mask[:, :-1]], 1)).view(-1)
        # print(input, reward, mask)
        output = - input * reward * mask
        # print(output.device, self.one.to(output.device))
        count = torch.max(torch.sum(mask), self.one.to(output.device).double()) # .to() change data type as utput.device
        output = torch.sum(output) / count
        #print('reward loss:', output)
        return output
...
class BaseTrainer:
    ...
    def load(self):
        ...
        # print('python/trainers/bse_trainer.py self.beam_size', self.beam_size)
    ...
    def load_extras(self):
        ...
        self.sc_learning = self.training_parameters.sc_learning # anwen hu 2020/11/11
        if self.sc_learning:
            self.cider_calculator = TextCapsCider()
            self.reward_loss_calculator = RewardCriterion()
    ...
    def train(self):
        ...
        # self.reward_loss_calculator = RewardCriterion()
        # if self.sc_learning:
        """if self.current_iteration >= 12000:
            self.sc_learning = True
            self.cider_calculator = TextCapsCider()
            print('start sc learning')"""
        # print('base_trainer self.current_iteration', self.current_iteration)
        while self.current_iteration < self.max_iterations and not should_break:
            ...
            for batch in self.train_loader:
                ...
                if self.current_iteration > self.max_iterations:
                    break
                report = self._forward_pass(batch)  # containg loss and score
    ...
    def _forward_pass(self, batch):
        ...
        # anwen hu
        if self.sc_learning:
            # model output contains scores, bleu, loss(calculated in base_model)
            model_output = self.model(prepared_batch, sc=True) # first greedy and then sample
            self.cider_calculator(prepared_batch, model_output)  # add reward to model_output
            # print('base_trainer.py model_output reward', model_output['reward'].size())
            # print('model_output sample seq log probs', model_output['sampleseqLogprobs'].size())
            assert len(list(model_output['losses'].keys())) == 1
            loss_key = list(model_output['losses'].keys())[0]
            model_output['losses'][loss_key] = self.reward_loss_calculator(model_output['sampleseqLogprobs'],
                                                                 model_output['prev_inds'].data,
                                                                 model_output['reward'])
        else:
            # model output contains scores, bleu, loss(calculated in base_model)
            model_output = self.model(prepared_batch, sc=False)
        # print('model output key', model_output.keys())
    ...
    def _logistics(self, report):
        # print('base_trainer.py current iteration %d, log_interval %d' % (self.current_iteration, self.log_interval))
        ...
        with torch.no_grad():
            ...
            while reporter.next_dataset():
                ...
                for batch in tqdm(dataloader):
                    ...
                    model_output = self.model(prepared_batch, beam_size=self.beam_size)
                    report = Report(prepared_batch, model_output)
                    # print(report.keys())
                    reporter.add_to_report(report, beam_size=self.beam_size)
